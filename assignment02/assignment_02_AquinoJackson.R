...
## Use `setwd()` if needed
setwd("/users/jackson_aquino/dsc520/")
...
colnames(voter_turnout_df1) <- voter_turnout_df1[1,]
voter_turnout_df1 <- voter_turnout_df1[-1,]
...
voter_turnout_df2 <- read_excel("data/G04ResultsDetail2004-11-02.xls","Voter Turnout")
voter_turnout_df2 <- voter_turnout_df2[-1,]
...
tables <- lapply(table_names, dbReadTable, conn = db, USE.NAMES = TRUE)
tables
