...
## `height` vs. `earn`
ggplot(heights_df, aes(x=heights_df$height, y=heights_df$earn)) + geom_point()
## `age` vs. `earn`
...
ggplot(heights_df, aes(x=height, y=earn, col=sex)) + xlab('Height (Inches)') + ylab('Earnings (Dollars)') + ggtitle('Height vs. Earnings') + geom_point()
...
## Create a histogram of the `earn` variable using `geom_histogram()`
## Use 10 bins
ggplot(heights_df) + geom_histogram(aes(x=earn),bins = 10)

# https://ggplot2.tidyverse.org/reference/geom_density.html
## Create a kernel density plot of `earn` using `geom_density()`
ggplot(heights_df) +  geom_density(aes(x=earn))
